...
function addToCart(goodsId, parentId) {
	...
	//alert( JSON.stringify( goods.toJSONString() ) );
	api.ajax({
		url : phpUrl + '/cart.php?step=add_to_cart',
		method : "post",
		data : {
			values : {
...
function addToCart_quick(goodsId, parentId) {
	...
	goods.spec = spec_arr;
	goods.goods_id = goodsId;
	goods.number = numbers;
	goods.parent = ( typeof (parentId) == "undefined") ? 0 : parseInt(parentId);
//	return;
	api.ajax({
		url : phpUrl + '/cart.php?step=cart',
	}, function(ret, err) {
				...
				created : function() {
					...
					if (self.itemsa == "") {
						...
						api.ajax({
							...
							addToCartResponse_quick(ret);
						});
					} else {
						for (var i = 0; i < self.itemsa.length; i++) {
							if (goods.goods_id == self.itemsa[i].goods_id) {
								numbers = 0;
								goods.number = numbers;
								api.ajax({
									url : phpUrl + '/cart.php?step=add_to_cart',
	...
	//Ajax.call(phpUrl+'/cart.php?step=add_to_cart', 'goods=' + goods.toJSONString(), addToCartResponse_quick, 'POST', 'JSON');
...
function addToCartResponse(result) {
	...
	api.execScript({
		name : 'root',
		frameName : 'cart',
		script : "getCartInfo();hehe();"
	});

	//consloe.log(result);e
	...
	if (result.error > 0) {
		...
		document.getElementById('carId2').innerHTML = result.cart_number;
...
function addToCartResponse_quick(result) {
	...
	if(result.error == 3 || result.error == 2){
		api.toast({
	...
	var cart_url = phpUrl + '/cart.php?step=cart';
...
function collect(goodsId) {
	...
	Ajax.call(phpUrl + '/user.php?act=collect', 'id=' + goodsId, collectResponse, 'GET', 'JSON');
...
function submit_div(goods_id, parentId) {
	...
	Ajax.call(phpUrl + 'cart.php?step=add_to_cart', 'goods=' + goods.toJSONString(), addToCartResponse, 'POST', 'JSON');
