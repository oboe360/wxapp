...
class TestController extends Controller {
    ...
    public function tuoke_fans(){
        $shop_id = session('shop_id');
        //查询普通用户
        $fans_list = D()->query("SELECT b.* FROM app_history a INNER JOIN app_user b on a.uid = b.uid WHERE a.shop_id = ".$shop_id." and user_rank = 1 GROUP BY uid limit 0,10");
        $fans_list_count = count(D()->query("SELECT b.* FROM app_history a INNER JOIN app_user b on a.uid = b.uid WHERE a.shop_id = ".$shop_id." and user_rank = 1 GROUP BY uid "));
        //查询会员用户
        $fans_list_vip = D()->query("SELECT b.* FROM app_history a INNER JOIN app_user b on a.uid = b.uid WHERE a.shop_id = ".$shop_id." and user_rank = 2 GROUP BY uid limit 0,10");
        $fans_list_vip_count = count(D()->query("SELECT b.* FROM app_history a INNER JOIN app_user b on a.uid = b.uid WHERE a.shop_id = ".$shop_id." and user_rank = 2 GROUP BY uid "));
        // dump($fans_list);die;
    ...
    public function tuoke_fans_pubu(){
        ...
        if(!empty($last_num)){
            //$last_num --;
            // $User = M("shop_user");
            $data = D()->query("SELECT b.* FROM app_history a INNER JOIN app_user b on a.uid = b.uid WHERE a.shop_id = ".$shop_id." and user_rank = ".$type." GROUP BY uid limit ".$last_num.",10");
            // dump($data);
            if($data){
    ...
    public function shop_lower(){
        $shop_id = session('shop_id');
        // $shop_id = 1;
        $data = D()->query("SELECT a.*, (SELECT COUNT(*) FROM `app_user` WHERE shop_id = a.id) AS count_user FROM `app_the_store` AS a WHERE a.`sj_id` = '{$shop_id}' ORDER BY a.add_time DESC LIMIT 10");
        for($i = 0; $i < count($data); $i++){
            $data[$i]['add_time'] = date('Y-m-d H:i', $data[$i]['add_time']);
        }
        // var_dump($data);
    ...
    public function shop_lower_return(){
        ...
        if(!empty($last_num)){
            //$last_num --;
            $data = D()->query("SELECT a.*, (SELECT COUNT(*) FROM `app_user` WHERE shop_id = a.id) AS count_user FROM `app_the_store` AS a WHERE a.`sj_id` = '{$shop_id}'  ORDER BY a.add_time DESC LIMIT {$last_num}, 10");
            //dump($data);
            if($data){
                for($i = 0; $i < count($data); $i++){
                    $data[$i]['add_time'] = date('Y-m-d H:i', $data[$i]['add_time']);
    ...
    public function performance_report()
    {
        if ($_REQUEST['type'] == '1') {
            ...
            $shop_id = session('shop_id');
            $report_list = $User
            ->where('shop_id = '.$shop_id)->limit($last_num,10)->select();
            ...
            $shop_id = session('shop_id');
            $report_list = $User
            ->where('shop_id = '.$shop_id)->limit(0,10)->select();
    ...
    public function performance_report_data()
    {
        ...
        if ($_REQUEST['type'] == '1') {
            ...
            ->where('shop_id = '.$shop_id.' and id='.$sn_id)->limit(0,10)->find();

            $User2 = M("shop_achievement_sn");
            $report_list_data2 = $User2
            ->JOIN("app_the_store on app_shop_achievement_sn.shop_id = app_the_store.id")
            ->where('app_shop_achievement_sn.shop_id in ('.$report_list_data['shop_list'].')')->limit($last_num,10)->select();
            ...
            $User = M("shop_achievement");
            $report_list_data = $User
            ->where('shop_id = '.$shop_id.' and id='.$_REQUEST['id'])->limit(0,10)->find();

            $User2 = M("shop_achievement_sn");
            $report_list_data2 = $User2
            ->JOIN("app_the_store on app_shop_achievement_sn.shop_id = app_the_store.id")
            ->where('app_shop_achievement_sn.shop_id in ('.$report_list_data['shop_list'].') and app_shop_achievement_sn.start_time >= '.$report_list_data['start_time'].' and app_shop_achievement_sn.end_time <= '.$report_list_data['end_time'])->limit(0,10)->select();
...
//---------------------店铺收益提现---------
 ...
 /*   	echo D('shop_bank_card')->getlastsql();
        ...
        $performance_money = D('shop_achievement')->where("`shop_id` = '{$shop_id}' and is_tixian = 0")->SUM('total_money');
        $performance_info = D('shop_achievement')->where("`shop_id` = '{$shop_id}' and is_tixian = 0")->select();
        // dump($performance_info);die;
        if (!empty($performance_info)) {
            foreach ($performance_info as $key => $value2) {
                $achievement_sn .= $value2['number'].',';
            }
        ...
        if($_POST['type'] == 'tixian'){
            ...
            if ($earnings['money'] > $tixis['0']['money']) {
                $data['error'] = '5';
                $data['str'] = '抱歉!提现出了小小问题,请重新提现或联系客户!';
                $this->ajaxReturn($data);exit;
            }
            ...
            $insert['money'] = sprintf("%.2f",($sum_money - ($sum_money * 0.01) - 1.5));
    	...
    	$earnings['account'] = sprintf("%.2f",($earnings['tixian'] - ($earnings['tixian'] * 0.01) - 1.5));
    	// dump($earnings);die;
    ...
    public function bank_card(){
    	...
    	if(!empty($_POST)){
    		...
    		if($bank_card){
    			...
    			$inser = D('shop_bank_card')->where("`id`='{$shop_id}'")->data($update)->save();
    		}else{
    ...
    public function get_small_app_qrcode($access_token,$path,$type) {
        ...
        curl_close($curlobj);
        $data = [ROOT_PATH."Public/Home/Test/apk/".$shop_id."_1.png"];
