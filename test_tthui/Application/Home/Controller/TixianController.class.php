...
class TixianController extends Controller
{
	...
	public function addUserCodeMessages(){
	 ...
	 //    //判断手机格式
		...
		if($data['uid']){
			//获取数据判断是否存在修改
			$list = $this->userBankCard->field('user_name,coop_bank,bank_name,bank_phone,bank_address,bank_city,id,bank_province')->where(array('uid'=>$data['uid']))->find();
			// echo json_encode($list);
			// echo json_encode($data);
			// exit;
			//如果存在数据，判断修改操作
			if($list){
				if($data['user_name'] == $list['user_name'] && $data['coop_bank'] == $list['coop_bank'] && $data['bank_name'] == $list['bank_name'] && $data['bank_phone'] == $list['bank_phone'] && $data['bank_address'] == $list['bank_address'] && $data['bank_city'] == $list['bank_city'] && $data['bank_province'] == $list['bank_province']){
					$param['code'] = 0;
		        	$param['msg'] = '信息未做修改';
		        	echo json_encode($param,JSON_UNESCAPED_UNICODE);
					exit;
				...
				$data['id'] = $list['id'];
				//开启事务，更新对应字段
				$this->userBankCard->startTrans();
				$bool = $this->userBankCard->save($data);
				if ($bool) {
					$tixian_bool = $this->updateTixianHistorys($data);
					if($tixian_bool){
						$param['code'] = 1;
		        		$param['msg'] = '保存信息成功';
		        		$this->userBankCard->commit();
					}else{
						$param['code'] = 0;
		        		$param['msg'] = '保存信息失败';
		        		$this->userBankCard->rollback();
					}		        	
		        	// return true;
		        } else {
		            $param['code'] = 0;
	...
	//获取用户是否需要修改提现信息
	public function getUpdateErrorTixian(){
		$data = I('param.');
		$data['status'] = 2;
		$bank_province = $this->tixian->where($data)->getField('bank_province');
		if($bank_province){
			return $this->ajaxReturn(array('code'=>0,'msg'=>'不需要修改'));
		}else{
			return $this->ajaxReturn(array('code'=>1,'msg'=>'需要修改'));
		}
	}
	//添加或修改身份认证信息
	...
	//修改已有提现信息
	//param array $data 银行卡信息数据
	private function updateTixianHistorys($data){
		$bool = $this->tixian->where(array('uid'=>$data['uid']))->save($data);
		return $bool;
	}
	//获取提现历史记录
