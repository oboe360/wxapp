 ...
 /*     echo D('shop_bank_card')->getlastsql();
        ...
        if($_POST['type'] == 'tixian_order' && $_POST['vehicle'] != ''){
            ...
            for($i = 0; $i < count($_POST['vehicle']); $i++){
                ...
                $ti_order_ls .= $_POST['vehicle'][$i].',';
            }
            $ti_order = trim($ti_order, ',');
            $earnings['ti_order'] = trim($ti_order_ls, ',');
            //$earnings['ti_order'] = implode(',', $_POST['vehicle']);
            $tixis = D()->query("SELECT SUM(`money`) as `money` FROM `app_shop_earnings_record` WHERE `shop_id` = '{$shop_id}' AND `is_tixian` = '0' AND `order_sn` IN({$tixis})");
            //dump($ti_order);
            ...
            //dump($ti_order);
            $ti_order = trim($ti_order, ',');
            $insert['all_order_sn']  = $ti_order;
            $tixis = D()->query("SELECT SUM(`money`) as `money` FROM `app_shop_earnings_record` WHERE `shop_id` = '{$shop_id}' AND `is_tixian` = '0' AND `order_sn` IN({$ti_order})");
            //dump($ti_order);
            ...
            $insert['poundage'] = sprintf("%.2f", ($insert['money'] * 0.01) + 1.5);
            $insert['age_money'] = sprintf("%.2f", $insert['money'] - $insert['poundage']);
            //dump($insert);exit;
            M()->startTrans();
            ...
            $arr['is_tixian'] = '0';
            $arr['shop_id'] = $shop_id;
            $ti_order = D('shop_earnings_record')->field('order_sn')->where($arr)->select();
            //dump($ti_order);
        ...
        //dump($earnings);
        $this->assign('other_config', session('other_config'));
    ...
    public function send()
    {
       ...
       /* 避免空验证码提交直接绕过验证 */
        ...
        // if($_REQUEST['yanzhengma'] != NULL){
        //   $sms = new SmsApi("LTAIDq6kVcwnKeGN", "8SjeIgPotgdAC5f2b2i1eUDFhu8OEX"); // 请参阅 https://ak-console.aliyun.com/ 获取AK信息
        //     $response = $sms->sendSms(
        //         "美俪能量", // 短信签名 环球美淘
        //         "SMS_141885235", // 短信模板编号
        //         $_REQUEST['mobile'], // 短信接收者
        //         Array (  // 短信模板中字段的值
        //             "code"=> $mobile_code
    ...
    public function check()
    {
        ...
        //验证上级推荐号是否存在
        $user_info2 = M("the_store")->where('id = '.$_REQUEST['recommendCode'])->select();
        if (empty($user_info2)) {
            echo 3;
            exit();
        }
