<?php
   ...
   * @return  void
   */
  function secondIntoApp($shop_id, $wxuser, $Common, $uid, $history, $user){
    ...
    //设置用户上下级关系
    if($uid){
      if($uid != $wxuser['sj_uid'] && $uid != $wxuser['uid'] && $uid !== 'false'){
        ...
        if($user_rank > 1){//如果用户为普通用户，则绑定该用户上级
          ...
          // $sj_user_rank=$Common->user_rank($uid);//获取分享用户的等级
          // if($sj_user_rank > 1){
          $shop_user_where['sj_uid'] = $sjuid;
          // }
        }
      }elseif($uid === 'false'){
        $shop_user_where['sj_uid'] = $Common->getBdUid($shop_id);
        if($shop_user_where['sj_uid'] == $wxuser['sj_uid']){
          unset($shop_user_where['sj_uid']);
        }
      }
    }
    //如果存在更新条件，则进行更新操作
    ...
    // file_put_contents('result.txt', json_encode($data)."\n", FILE_APPEND);
      // file_put_contents('result.txt', json_encode($shop_id)."\n", FILE_APPEND);
   ...
   * @return  void
   */
  function firstIntoApp($shop_id, $wxuser, $userInfo, $Common, $uid, $history, $user, $openid, $session_key,$function){
    // dump($function);
    // exit;
    if($uid){//判断是否是通过别人分享的产品进入小程序
      ...
      // file_put_contents('result.txt', json_encode($userInfo)."\n", FILE_APPEND);
