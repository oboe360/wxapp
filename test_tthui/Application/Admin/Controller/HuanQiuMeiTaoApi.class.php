 ...
 */
class HuanQiuMeiTaoApi extends BaseController{
		private $appid;
    	private $appkey;
		...
		function  __construct() {
				$list = $this->app();
       			...
       			$this->appkey = $list['appkey'];
       			define("API_ACCOUNT",$this->appid);//
       			define("API_PASSWORD",$this->appkey);
	...
	public function app() {
		!is_null($this -> appuser) ? : $this -> appuser = D('config');
		//美淘商品接口参数
		$row = $this->appuser->field('appid,appkey')->select();
		//dump($row);die;
	 ...
	 * @param string $action 		$action=='shipping_fee' 请求数据行为:判断可以是进行那个模块的数据请
	 */
	public function order_info($orderdata,$action = 'order_info'){
		$postArr = array ( 
          'appid' => base64_encode(API_ACCOUNT),
          'appkey' => MD5(API_PASSWORD),
          'signature' => $this->MD_5(self::API_SINGNATURE.API_PASSWORD),    
